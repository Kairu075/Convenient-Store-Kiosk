...
public class SalesReport {
    ...
    private static ArrayList<SaleTransaction> transactions = new ArrayList<>();
    ...
    private JPanel createSummaryPanel(Date startDate) {
        ...
        ArrayList<SaleTransaction> filteredTransactions = filterTransactionsByDate(startDate);
        ...
        int totalItems = filteredTransactions.stream()
                .mapToInt(t -> t.getItems().values().stream().mapToInt(Integer::intValue).sum())
                .sum();
        long discountedSales = filteredTransactions.stream()
                .filter(SaleTransaction::isDiscountApplied)
                .count();
    ...
    private JScrollPane createTransactionsPanel(Date startDate) {
        ...
        for (SaleTransaction transaction : filteredTransactions) {
            int itemCount = transaction.getItems().values().stream().mapToInt(Integer::intValue).sum();
    ...
    private JScrollPane createItemsPanel(Date startDate) {
        ...
        for (SaleTransaction transaction : filteredTransactions) {
            for (Map.Entry<String, Integer> entry : transaction.getItems().entrySet()) {
                ...
                double price = transaction.getPrices().get(item);
     ...
     */
    private ArrayList<SaleTransaction> filterTransactionsByDate(Date startDate) {
        if (startDate == null) {
            return new ArrayList<>(transactions);
        }
        
        ArrayList<SaleTransaction> filtered = new ArrayList<>();
        for (SaleTransaction transaction : transactions) {
            if (transaction.getDate().after(startDate) || transaction.getDate().equals(startDate)) {
